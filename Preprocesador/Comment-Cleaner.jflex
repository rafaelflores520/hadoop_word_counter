...
format = &gt
botcomm = The{espacio}Bitcoin{espacio}tip{espacio}for{espacio}{numero}+{espacio}bits{espacio}\(\${numero}+.{numero}+\){espacio}has{espacio}been{espacio}collected{espacio}by{espacio}\*[a-zA-z_-]+\* 
botcomm2 = ({user}\,{espacio}[a-zA-z_-]+{espacio}wants{espacio}to{espacio}send{espacio}you{espacio}a{espacio}Bitcoin{espacio}tip{espacio}for{espacio}{numero}{espacio}bits) 
botcomm3 = (Follow{espacio}me{espacio}to{espacio}\*\*\[collect{espacio}it\]\({url}\)\.\*\*)
noColumn = {numero}\,{fecha}?{espacio}?{hora}?\,{fecha}?\,({usuario}|\[deleted\])?\,{usuario}?\,{numero}?\,\-?{numero}\.{numero}?\,{numero}?\,
...
<YYINITIAL>{
	...
	{espaciot} 		{writer(yytext());}
	{espacion} 		{writer(yytext());}
	{espacio} 		{writer(" ");}
	. 				{}
}